...
- It provides quick access to the Pine Script™ Reference Manual popup when you :kbd:`ctrl` + :kbd:`click` / :kbd:`cmd` + :kbd:`click` on Pine Script™ keywords.
...
- We now use the `ta.macd() <https://www.tradingview.com/pine-script-reference/v5/#fun_ta{dot}macd>`__ 
  Pine Script™ built-in to calculate our MACD, which saves us three line and makes our code easier to read.
...
Line 3: ``fastInput = input(12, "Fast length")``
    ...
    Note that, as our :ref:`Pine Script™ Style Guide <PageStyleGuide>` recommends, we add ``Input`` to the end of the variable's name to remind us, later in the script,
